...
void Events::wm_create(std::function<int(wm::Create)> cb) {
	_inis.emplace_back(WM_CREATE, cb);
}

void Events::wm_init_dialog(std::function<bool(wm::InitDialog)> cb) {
	_inis.emplace_back(WM_INITDIALOG, cb);
}

void Events::wm(UINT msg, std::function<LRESULT(wm::Msg)> cb) {
	#ifdef _DEBUG
...
bool Events::process_all(wm::Msg procMsg) const {
	...
	switch (procMsg.wm) {
		...
		case WM_COMMAND: {
			...
			for (auto& cmd : _cmds) {
				if (cmd.cmdId == msgCmd.control_id() && cmd.notifCode == msgCmd.control_notif_code()) {
					cmd.cb();
					atLeastOne = true;
				}
			}
...
std::optional<LRESULT> Events::process_last(wm::Msg procMsg) const {
