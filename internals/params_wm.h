...
#define WINLAMB_EMPTYWM(sname) \
	...
	struct askcbformatname : public params {
		...
		wchar_t* buffer() const   { return reinterpret_cast<wchar_t*>(this->lParam); }
	};
	...
	struct devmodechange : public params {
		...
		const wchar_t* device_name() const { return reinterpret_cast<const wchar_t*>(this->lParam); }
	};
	...
	struct getdlgcode : public params {
		...
		bool is_query() const  { return this->lParam == 0; }
		MSG* msg() const       { return this->is_query() ? nullptr : reinterpret_cast<MSG*>(this->lParam); }
	};
	...
	struct initmenupopup : public params {
		...
		UINT  first_menu_item_id() const { return GetMenuItemID(this->hmenu(), 0); }
	};
	...
	struct settext : public params {
		...
		const wchar_t* text() const { return reinterpret_cast<const wchar_t*>(this->lParam); }
	};
	struct settingchange : public params {
		settingchange(const params& p) : params(p) { }
		const wchar_t* string_id() const           { return reinterpret_cast<const wchar_t*>(this->lParam); }
		bool           is_policy() const           { return !lstrcmp(this->string_id(), L"Policy"); }
		bool           is_locale() const           { return !lstrcmp(this->string_id(), L"intl"); }
		bool           is_environment_vars() const { return !lstrcmp(this->string_id(), L"Environment"); }
	};
