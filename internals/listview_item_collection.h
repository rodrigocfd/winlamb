...
public:
	...
	explicit listview_item_collection(listviewT* pList)
		: _list(*pList) { }

	_itemT operator[](size_t itemIndex) const {
	...
	_itemT add(const wchar_t* caption, int imagelistIconIndex = -1, size_t positionIndex = _itemT::npos) {
		LVITEMW lvi{};
	...
	_itemT add(const std::wstring& caption, int imagelistIconIndex = -1, size_t positionIndex = _itemT::npos) {
	...
	listviewT& remove_all() {
		ListView_DeleteAllItems(this->_list.hwnd());
		return this->_list;
	}

	_itemT find(const wchar_t* caption) const {
		LVFINDINFO lfi{};
	...
	_itemT find(const std::wstring& caption) const {
	...
	listviewT& select(const std::vector<size_t>& indexes) const {
